...
<Callout type="warn">
  `OIDC_PROVIDER_ID` and `NEXT_PUBLIC_OIDC_PROVIDER_ID` must be set to the same value. The first is
  read server-side, the second by the client-side sign-in flow, and a mismatch breaks the redirect
...
For more details on AI configuration, see [AI Generation](../features/ai-generation).
