...
Set the project's **function region** to the region your database lives in.
Functions default to `iad1` (Washington, D.C.), so a European database means
every query pays a transatlantic round trip on every request.
...
- `installCommand: pnpm install --frozen-lockfile` — fail on a stale lockfile
  instead of silently resolving different versions than CI did.
- `ignoreCommand: npx turbo-ignore <app>` — skip the build when the commit
  didn't touch that app or its dependencies, so a docs-only change doesn't
