...
Respondeo uses unit tests, integration tests that run against real backing
services, and k6 load tests.
...
That script starts Postgres and Valkey, applies migrations, and runs the full
...
The load test script is located at `apps/web/docs/tests/loadtest.js`.
...
Edit `apps/web/docs/tests/loadtest.js`:
