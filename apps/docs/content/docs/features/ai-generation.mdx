...
<Callout type="warn">
  These limits are what stands between an open sign-up and an unbounded AI provider bill, so they
  need a shared counter to be real. Configure `REDIS_URL` or `VALKEY_URL` in any multi-instance or
  serverless deployment — otherwise each instance counts separately and the global limit scales with
  your traffic. See [Caching](/features/caching).
</Callout>
